...
export default async function handler(
  ...
  try {
    ...
    const recommendedAgent = availableAgents.find(
      (agent) => agent.id === aiResult.recommendation?.recommendedAgentId
    );
    const alternativeAgents =
      aiResult.recommendation?.alternativeAgents?.map((alt) => ({
        ...alt,
